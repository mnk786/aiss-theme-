...
function aiss_get_categories()
{
	...
	$categories = get_terms('category', $argv);
	...
	$speed = '10';
	 foreach ($categories as $category){
	 	 if( $category->name == 'About') continue;
	 	 if( $i > 3 ) break;
...
function aiss_home_fitness(){
	...
	if( $fitness_query->have_posts() ) :
	  ...
	  if( $i > 1) break;
			...
			$output .= '<section class="fitness" style="background-image: url('.$src[0].');"><div class="container">';
...
function aiss_home_summer(){
	...
	if( $summer_query->have_posts() ) :
		while ($summer_query->have_posts()) : $summer_query->the_post();
	          if( $i > 1) break;
			$src =  wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()), 'full', true );
			$output .= '<section class="summer" style="background-image: url('.$src[0].');"><div class="container">';
...
function subcat($id)
{
	$sub_args = array(
		'taxonomy'      => 'category',
		'parent'        => $id, // get child categories
		'orderby'       => 'id',
		'order'         => 'ASC',
		'hierarchical'  => 1,
		'pad_counts'    => 0,
